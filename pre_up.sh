...
# DEBUGGING
  ...
  # recommend running as a systemd service
    ...
    then # no systemd service
         echo "No systemd service exists" |
  ...
  # vpn active
       ...
       # GUI notification
         _is_not_tty \
           && _pia_notify 'Disconnected from '"${REGION_NAME}"' ' 5000 "pia_off_48x48.png"
  ...
  # No, restore valid nameservers
    then _logger "restoring DNS nameservers"
       ...
       # There's a copy of /etc/resov.conf saved at system start by pre_up.sh
              cp -v /storage/.cache/starting_resolv.conf /run/connman/resolv.conf |
              tee >(_logger)
